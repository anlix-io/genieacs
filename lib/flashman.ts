...
export const sendFlashmanInformRequest = async function (
  ...
  const body = {
    ...
    firmwareVersion:
      parameterMap["InternetGatewayDevice.DeviceInfo.SoftwareVersion"],
    hardwareVersion:
      parameterMap["InternetGatewayDevice.DeviceInfo.HardwareVersion"],
  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      reject(Error('POST on Flashman path "/acs/device/inform" timed out'));
    }, 10000);
      ...
      (res) => {
        ...
        res.on("end", () => {
          ...
          if (res.statusCode < 200 || res.statusCode >= 300) {
            return reject(
              Error(
                `Flashman response from inform: ${res.statusCode}\n${responseBody}`
              )
            );
          } else {
